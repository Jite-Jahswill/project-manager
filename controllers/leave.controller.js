...
module.exports = {
  ...
  async getAllLeaves(req, res) {
    try {
      ...
      if (startDate) where.startDate = { [db.Sequelize.Op.gte]: new Date(startDate) };
      if (endDate) where.endDate = { [db.Sequelize.Op.lte]: new Date(endDate) };
  ...
  async getLeavesByUserId(req, res) {
    try {
      ...
      if (count === 0) return res.status(404).json({ message: "No leaves found for this user" });
      ...
      res.status(500).json({ message: "Error fetching leaves by user", details: error.message });
    }
  },
  ...
  async getLeaveById(req, res) {
    try {
      ...
      res.status(500).json({ message: "Error fetching leave", details: error.message });
    }
  },

  // Update leave using raw MySQL query
  async updateLeave(req, res) {
    try {
      ...
      const updates = [];
      if (startDate) updates.push(`startDate='${new Date(startDate).toISOString().slice(0, 19).replace("T", " ")}'`);
      if (endDate) updates.push(`endDate='${new Date(endDate).toISOString().slice(0, 19).replace("T", " ")}'`);
      if (reason) updates.push(`reason='${reason.replace(/'/g, "\\'")}'`);
      updates.push(`updatedAt=NOW()`);

      if (updates.length > 0) {
        const query = `UPDATE Leaves SET ${updates.join(", ")} WHERE id=${parseInt(id)}`;
